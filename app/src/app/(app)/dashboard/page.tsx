...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (!token) return;
    Promise.all([
      api.issues.list(token, { limit: "5", page: "1" }),
          ...
          <div className="relative bg-surface-container-lowest p-6 rounded-xl border border-secondary/10 shadow-sm">
            ...
            <ul className="space-y-3">
              {loading ? (
                ...
                `${issues.filter(i => i.status === "done").length} issues done in current view`,
        ...
        <section className="space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <h3 className="font-headline font-extrabold text-xl tracking-tight">Recent Issues</h3>
              <span className="px-2 py-0.5 rounded-full bg-surface-container-high text-on-surface-variant text-xs font-bold">{issues.length}</span>
            </div>
            <div className="flex gap-1 p-1 bg-surface-container-low rounded-lg w-fit text-sm font-medium text-on-surface-variant">
              <span className="px-3 py-1.5">In Progress ({inProgress})</span>
              <span className="px-3 py-1.5">To Do ({todo})</span>
            </div>
          </div>
